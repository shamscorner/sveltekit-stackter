...
async function adminLogin(pb: PocketBase, options: PocketBaseAdapterOptions): Promise<any> {
	...
	return {
		...
		async getUser(id) {
			try {
				...
				const pb_user = await client.collection('users').getOne<PocketBaseUser>(id);
		...
		async getUserByAccount({ providerAccountId, provider }) {
			try {
				...
				const pb_user = await client.collection('users').getOne<PocketBaseUser>(pb_account.userId);
		...
		async updateUser(user) {
			...
			const pb_user = await client.collection('users').update<PocketBaseUser>(user.id, to(user));

			return from(pb_user, true);
		},
		async linkAccount(account) {
			const pb_account = await client.collection('accounts').create<PocketBaseAccount>({
				...account
			});
		...
		async createSession(session) {
			const pb_session = await client.collection('sessions').create<PocketBaseSession>({
				...session,
		...
		async getSessionAndUser(sessionToken) {
			try {
				...
				const pb_user = await client.collection('users').getOne<PocketBaseUser>(pb_session.userId);
		...
		async updateSession(session) {
			...
			const record = await client
				.collection('sessions')
				.getFirstListItem<PocketBaseSession>(`sessionToken="${session.sessionToken}"`);
		...
		async useVerificationToken({ identifier, token }) {
			try {
				...
				const success = await client.collection('verificationTokens').delete(pb_veriToken.id);
